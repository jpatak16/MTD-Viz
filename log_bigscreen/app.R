...
ui = navbarPage("2023 NBA Trade Deadline Competition", fluid = TRUE,
                tabPanel("Transaction Log",
                         ...
                         fluidRow(column(12, h1(span(textOutput("countdown"), style = 'font-size: 40px; font-weight: bold;'))), align = 'center'),
                         fluidRow(column(6, gt_output("TransactionLog1")),
                                  column(6, gt_output("TransactionLog2")))),
                )
...
server <- function(input, output, session) {
  ...
  sec_rem = reactiveVal(1699059600- as.numeric(Sys.time()))
  
  hours_rem = reactive(ifelse(floor(sec_rem() / 3600) %>% as.numeric() > 0, floor(sec_rem() / 3600) %>% as.numeric(), 0))
  
  mins_rem = reactive(ifelse(floor((sec_rem() - (hours_rem()*3600)) / 60) %>% as.numeric() >= 0, floor((sec_rem() - (hours_rem()*3600)) / 60) %>% as.numeric(), 0))
  
  time_to_sub = reactive(hours_rem()*3600 + mins_rem()*60)
  
  sec_rem_fix = reactive(ifelse(floor(sec_rem() - time_to_sub()) >= 0, floor(sec_rem() - time_to_sub()), 0))
  
  output$countdown = renderText(paste0("Time Remaining: ", hours_rem(), " Hours, ", mins_rem(), " Minutes, ", sec_rem_fix(), " Seconds!"))
  ...
  ids_1 = reactive(transaction_log_fixed() %>% arrange(desc(trans_3d_id), compnum) %>%
                     pull(trans_ID) %>% unique())
  ids_1_filtered = reactive(ids_1()[c(TRUE, FALSE)])
  
  tl_1 = reactive(
    transaction_log_fixed() %>% filter(trans_ID %in% ids_1_filtered()) %>%
      gt() %>%
      ...
      cols_hide(columns = c(trans_ID, compnum, trans_3d_id, to_team, first, judge_note, status)) %>%
  ...
  ids_2_filtered = reactive(ids_1()[c(FALSE, TRUE)])
  
  tl_2 = reactive(
    transaction_log_fixed() %>% filter(trans_ID %in% ids_2_filtered()) %>%
      gt() %>%
      gt_img_rows(columns = logo, height = 75) %>%
      gt_theme_pff() %>%
      #dark borders between trades
